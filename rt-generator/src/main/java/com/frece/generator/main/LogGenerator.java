package com.frece.generator.main;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.frece.generator.util.RanOpt;
import com.frece.generator.util.RandomDate;
import com.frece.generator.util.RandomNum;
import com.frece.generator.util.RandomOptionGroup;
...
//    int eventNum=200000 ;
    ...
    public LogGenerator(String startTimeString , String endTimeString, int startupNum, int eventNum) {
        try {
    ...
    String initStartupLog( ){
     ...
     `area` string COMMENT '城市' */
        ...
        String mid= "mid_"+ RandomNum.getRandInt(1000,2000);
        String uid=""+ RandomNum.getRandInt(1000,3000);
    ...
    public static void genLog() {
        ...
        logGenerator.startupNum = 2000000;
        for (int i = 0; i < logGenerator.startupNum; i++) {
            String startupLog = logGenerator.initStartupLog();
            logGenerator.sendLog(startupLog);
            while (! logGenerator.isQuitGroup.getRandomOpt().getValue()) {
                String eventLog = logGenerator.initEventLog(startupLog);
                logGenerator.sendLog(eventLog);
                try {
                    Thread.sleep(200);
